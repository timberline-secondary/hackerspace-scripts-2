...
def check_for_disconnected_computers(password=None):
    ...
    for i in computer_numbers:
        hostname = f'tbl-h10-{i}'
        if utils.host_exists(hostname, verbose=False):
            utils.print_success(f"{hostname} found.")
        else:
            utils.print_error(f"{hostname} missing.")

    ## This doesn't work with new DNS Setup.  Just ping instead.
